...
Fire-and-forget your coding tasks → Claude Code builds it in the background → you get a rich Discord notification when it's done.
...
- 📱 **Discord notifications** — Rich completion reports with test results, file listings, duration
- 🔄 **Three-layer callback** — Discord message → webhook wake → heartbeat fallback
...
| `--group` | `-g` | | Discord group ID for notifications |
...
| `--callback-group` | | | Discord group for callback |
| `--callback-dm` | | | Discord user ID for DM callback |
| `--callback-account` | | | Discord bot account name |
...
- Discord notifications via `openclaw message send`
