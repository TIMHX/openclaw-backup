...
#   -g, --group ID           Discord group ID for result delivery
...
#   --callback-group ID      Discord group for callback (dispatching agent's group)
#   --callback-dm ID         Discord user ID for DM callback
#   --callback-account NAME  Discord bot account name for DM callback
#
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -g|--group) DISCORD_GROUP="$2"; shift 2;;
...
jq -n \
    ...
    '{task_name: $name, discord_group: $group, callback_group: $callback_group, callback_dm: $callback_dm, callback_account: $callback_account, callback_session: $session, prompt: $prompt, workdir: $workdir, started_at: $ts, agent_teams: ($agent_teams == "1"), status: "running"}' \
    > "$META_FILE"

echo "📋 Task metadata written: $META_FILE"
echo "   Task: $TASK_NAME"
echo "   Group: ${DISCORD_GROUP:-none}"
