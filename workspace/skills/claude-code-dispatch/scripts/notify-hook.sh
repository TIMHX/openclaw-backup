...
#   3. Sends rich Discord notifications via OpenClaw CLI
...
if [ -f "$META_FILE" ]; then
    ...
    if [ "$META_AGE" -gt 7200 ]; then
        ...
        DISCORD_GROUP=$(jq -r '.discord_group // ""' "$META_FILE" 2>/dev/null || echo "")
        ...
        log "Meta: task=$TASK_NAME group=$DISCORD_GROUP age=${META_AGE}s"
    fi
fi
...
if [ -n "$DISCORD_GROUP" ] && [ -n "$OPENCLAW_BIN" ]; then
...
${FILE_TREE}"
    ...
    "$OPENCLAW_BIN" message send \
        --channel discord \
        --target "$DISCORD_GROUP" \
        --message "$MSG" 2>/dev/null && log "Sent Discord notification" || log "Discord send failed"

    # Callback to dispatching agent's group
    if [ -n "$CALLBACK_GROUP" ] && [ "$CALLBACK_GROUP" != "$DISCORD_GROUP" ]; then
...
📝 ${SUMMARY}"
        ...
        "$OPENCLAW_BIN" message send \
            --channel discord --target "$CALLBACK_GROUP" \
    ...
    if [ -n "$CALLBACK_DM" ]; then
        CALLBACK_MSG="🔔 *Task Complete*: \`${TASK_NAME}\` ${STATUS_EMOJI}"
        [ -n "$DURATION" ] && CALLBACK_MSG="${CALLBACK_MSG} (${DURATION})"
        DM_CMD=("$OPENCLAW_BIN" message send --channel discord --target "$CALLBACK_DM" --message "$CALLBACK_MSG")
...
jq -n \
    ...
    '{task_name: $task, discord_group: $group, timestamp: $ts, summary: $summary, processed: false}' \
...
WAKE_TEXT="[CLAUDE_CODE_DONE] task=${TASK_NAME} status=done group=${DISCORD_GROUP:-none} ts=$(date -Iseconds)"
