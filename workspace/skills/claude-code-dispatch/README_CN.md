...
把编码任务丢给 Claude Code → 它在后台自动构建 → 完成后你收到一条 Discord 通知，包含测试结果、文件列表、耗时等详细信息。
...
- 📱 **Discord 通知** — 丰富的完成报告（测试结果、文件列表、耗时、Agent 分工）
- 🔄 **三重回调保障** — Discord 消息 → webhook 唤醒 → 心跳兜底
...
| `--model` | | | 模型覆盖 |
| `--callback-group` | | | 回调 Discord 群组 |
| `--callback-dm` | | | 回调 Discord 用户 ID（DM） |
...
- 通过 `openclaw message send` 发送 Discord 通知
